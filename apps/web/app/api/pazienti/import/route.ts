...
export async function POST(request: Request) {
  ...
  try {
    ...
    let { studioId, medicoId, pazienti: righe } = body

    // Isolamento dati e determinazione Studio / Medico in base al ruolo
    if (auth.session.ruolo === 'medico') {
      ...
      if (!medicoId) {
        return NextResponse.json(
          { error: 'Seleziona il Medico Curante a cui associare il dataset' },
          { status: 400 }
        )
      }
    ...
    for (let i = 0; i < righe.length; i++) {
      ...
      const email = riga.email?.trim().toLowerCase() || null
      ...
      if (!/^[A-Z0-9]{16}$/.test(cf)) {
        errori.push({
          riga: i + 1,
          cf,
          errore: 'Il Codice Fiscale deve contenere esattamente 16 caratteri alfanumerici',
        })
        continue
      }
      ...
      // Se email presente, verifica se già usata
      if (email) {
        const [esistenteEmail] = await db
          .select({ id: pazienti.id })
          .from(pazienti)
          .where(eq(sql`lower(${pazienti.email})`, email))
          .limit(1)
        ...
        if (esistenteEmail) {
          errori.push({
            riga: i + 1,
            cf,
            errore: `L'email ${email} è già usata da un altro paziente`,
          })
          continue
        }
      }
